...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    if ([textField.accessibilityLabel isEqual: @"heightField"]) {
        if ([textField.text length] == 0) {
            textField.text = [NSString stringWithFormat:@"%d' %ld\"", ([self.heightCell.heightPicker selectedRowInComponent:0] + 3), (long)[self.heightCell.heightPicker selectedRowInComponent:1]];
        }
    }
    
    if ([textField.accessibilityLabel isEqual: @"weightField"]) {
        if ([textField.text length] == 0) {
            textField.text = [NSString stringWithFormat:@"%d lbs", ([self.weightCell.weightPicker selectedRowInComponent:0] + 100)];
        }
    }
    return YES;
}
